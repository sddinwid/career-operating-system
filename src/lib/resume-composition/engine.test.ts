      ...
      certifications: [
        {
          ...
          issuer: "AWS",
          awardDate: { raw: "2025", normalized: "2025", precision: "YEAR" },
          expirationDate: null,
          status: "CURRENT",
        ...
        rolesToEmphasize: [
          {
            ...
            relevantTechnologies: ["Python"],
            relevantAccomplishments: ["Improved throughput by 20 percent."],
            emphasisReason: "Strong Python support."
          }
        ],
        projectsToConsider: [
          {
...
describe("resume composition engine", () => {
  it("creates employer-facing content with provenance, summary, and sections", () => {
    const plan = buildStructuredResumePlan(buildPlanningInput());
    const composition = buildResumeComposition({
      runId: "resume-composition-1",
      workspaceId: "workspace-1",
      structuredResumeVersionId: "structured-resume-1",
      structuredResumePlan: plan,
      careerProfileVersionId: "career-version-1",
      careerProfileSourceChecksum: "career-source-checksum-1",
      careerProfileContent: buildPlanningInput().careerProfileContent,
      createdAt: "2026-07-17T13:00:00.000Z",
      inputChecksum: "resume-composition-checksum"
    });

    expect(composition.header.some((entry) => entry.included && entry.field === "EMAIL")).toBe(true);
    expect(composition.professionalSummary.sentenceCount).toBeGreaterThanOrEqual(2);
    expect(composition.skillsGroups.length).toBeGreaterThan(0);
    expect(composition.professionalExperience[0]?.bullets.length).toBeGreaterThan(0);
    expect(composition.selectedProjects[0]?.projectOnlyDisclosure).toContain("Project-only");
    expect(composition.education.length).toBe(1);
    expect(composition.certifications.length).toBe(1);
    expect(composition.professionalSummary.sentences[0]?.provenance.templateId).toContain("summary.");
  });
});
