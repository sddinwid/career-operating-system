...
async function createWorkspace() {
  const workspace = await prisma.workspace.create({
    data: {
      name: `Evidence Scoring Workspace ${randomUUID()}`,
      userSettings: {
        create: [
          {
            key: "defaultTimeZone",
...
async function prepareRetrievalRun(workspaceId: string) {
...
describe("evidence scoring service", () => {
  ...
  it("creates an immutable scoring run and reuses it for identical inputs", async () => {
    const workspace = await createWorkspace();
    const prepared = await prepareRetrievalRun(workspace.id);
    ...
    const first = await scoreRetrievedEvidence(
      workspace.id,
      { evidenceRetrievalRunId: prepared.retrievalRunId },
      prisma
    );
    const second = await scoreRetrievedEvidence(
      workspace.id,
      { evidenceRetrievalRunId: prepared.retrievalRunId },
      prisma
    );
    const stored = await parseStoredEvidenceScoringRun(workspace.id, first.run!.id, prisma);
    const retrievalRunAfter = await prisma.evidenceRetrievalRun.findUniqueOrThrow({
      where: { id: prepared.retrievalRunId }
    });
    ...
    expect(stored.run.evidenceRetrievalRunId).toBe(prepared.retrievalRunId);
    ...
    expect(stored.result.summary.requirementsScored).toBeGreaterThan(0);
    expect(retrievalRunAfter.id).toBe(prepared.retrievalRunId);
  ...
  it("creates a new scoring run when the retrieval input changes", async () => {
    const workspace = await createWorkspace();
    const prepared = await prepareRetrievalRun(workspace.id);
    ...
    expect(secondScore.duplicate).toBe(false);
    expect(secondScore.run?.id).not.toBe(first.run?.id);
    expect(secondScore.run?.evidenceRetrievalRunId).toBe(secondRetrieval.run?.id);
  });

  it("rolls back the scoring row when persistence fails after creation", async () => {
    const workspace = await createWorkspace();
    const prepared = await prepareRetrievalRun(workspace.id);
    ...
    ).rejects.toThrow(/simulated evidence scoring persistence failure/i);

    const count = await prisma.evidenceScoringRun.count({
