...
describe("cover-letter DOCX rendering", () => {
  it("builds and validates an ATS-friendly DOCX artifact", async () => {
    const buffer = await buildCoverLetterDocxBuffer(model);
    const validation = await validateCoverLetterDocxBuffer(buffer, model);

    expect(buffer.byteLength).toBeGreaterThan(0);
    expect(validation.valid).toBe(true);
    expect(validation.missingEntries).toEqual([]);
    expect(validation.missingSnippets).toEqual([]);
    expect(validation.forbiddenFragments).toEqual([]);
  });
});
