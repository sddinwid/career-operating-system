...
- July 22, 2026 corrective note: parser version `m3.2.6` and classifier version `m3.3.3` decompose Marathon-style compound education or experience and tooling or certification statements into atomic reviewed items, preserve equivalency modifiers, exclude compensation leakage from requirements, promote conservative contextual role-summary extraction, recognize reusable Skyflow `You have` and `You will` section headings, and keep overview marketing paragraphs out of company and role metadata without creating a new milestone
