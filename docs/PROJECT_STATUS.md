...
- Deterministic job-description parsing with immutable parse runs, parser-version idempotency, diagnostics, and read-only analysis
  ...
  - Verified corrective update on July 22, 2026: parser version `m3.2.6` now decomposes compound Marathon-style education, experience, methodology, tooling, and certification text into atomic requirement records, preserves shared equivalency modifiers, extracts conservative contextual role-summary items, excludes compensation boilerplate from candidate requirements, recognizes reusable Skyflow `You have` and `You will` headings, and keeps overview marketing paragraphs out of company and role metadata
